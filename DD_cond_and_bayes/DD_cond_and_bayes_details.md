...
- Possible playground — a reliability diagram with a single distortion/temperature slider that bows the curve off the diagonal while ECE updates live; a lab that runs an LLM classifier on a cached dataset (`goemotions` / `bitext_customer_support`) and reveals its overconfidence on real data. (Name it distinctly from the existing `judge_noise_and_bias`, which calibrates a *judge's thresholds* — a different sense of the word.)
